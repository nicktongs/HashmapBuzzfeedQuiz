...
public class Quiz {
    ...
    // file where overall popularity is stored
    static final String STATS_FILE = "category_stats.csv";

    public static void main(String[] args) throws Exception {
       
        HashMap<String, Integer> popularityMap = loadPopularityMap(STATS_FILE);

        //Categories
        Category GE = new Category("German Empire",
                "You are disciplined, organized, and driven by honor and achievement."
                        + " You like structure, efficiency, and planning ahead.");
        Category BE = new Category("British Empire",
                "You value stability, tradition, and influence at a distance. You stay calm under pressure"
                        + " and prefer thoughtful action, trying to mastermind any situation");
        Category FE = new Category("French Empire",
                "You are creative, expressive, and proud. You care most about style, culture, and"
                        + " making a strong personal impression.");
        Category RE = new Category("Russian Empire",
                "You are intense, emotional, and driven by big ideals and beliefs."
                        + "You may feel things deeply and transform over time.");
        Category AH = new Category("Austria-Hungary",
                "You balance many different sides of yourself. You care about harmony and connection,"
                        + " even when it is hard to keep everything together.");
        Category USA = new Category("United States of America",
                "You are optimistic, adaptable, and independent. You like new ideas, choice, and the freedom"
                        + " to find your own path");

        // 3. Create Questions (once)
        ...
        while (playAgain == 1) {
            // reset game points before each run
            GE.resetPoints();
            BE.resetPoints();
            FE.resetPoints();
            RE.resetPoints();
            ...
            System.out.println("If you were a 20th century Great Power, you'd be " + result.label + ". ");
            System.out.println(result.description);

            // update popularity map for this run and show stats
            logResult(popularityMap, result.label);

     // show most popular overall so far
     String overall = getMostPopularOverall(popularityMap);
     if (overall != null) {
         System.out.println("Most popular overall so far: " + overall);
      }

     // then show stats
      System.out.println("\nOverall category popularity (CSV):");
        printMapAsCSV(popularityMap);

System.out.println("\nOverall category popularity (JSON):");
printMapAsJSON(popularityMap);
        ...
        // after loop, save popularity
        savePopularityMap(STATS_FILE, popularityMap);
    ...
    // ===== HashMap-related helper methods =====

    // Increment the count for a category label
    public static void logResult(HashMap<String, Integer> map, String label) {
        int oldCount = map.getOrDefault(label, 0);
        map.put(label, oldCount + 1);
    }

    // Find the most popular category label overall from the popularityMap
public static String getMostPopularOverall(HashMap<String, Integer> map) {
    ...
    // Print map as CSV to console
    public static void printMapAsCSV(HashMap<String, Integer> map) {
        System.out.println("categoryLabel,totalWins");
        for (String key : map.keySet()) {
            int val = map.get(key);
    ...
    public static void printMapAsJSON(HashMap<String, Integer> map) {
        System.out.println("[");
        int i = 0;
        int size = map.size();
        for (String key : map.keySet()) {
            int val = map.get(key);
            System.out.print("  {\"categoryLabel\": \"" + key + "\", \"totalWins\": " + val + "}");
            if (i < size - 1) {
                System.out.print(",");
            }
            System.out.println();
    ...
    // Load map from CSV file: categoryLabel,totalWins
    public static HashMap<String, Integer> loadPopularityMap(String filename) {
        HashMap<String, Integer> map = new HashMap<>();
        File f = new File(filename);
        if (!f.exists()) {
            return map; // empty if no file yet
        }
        try {
    ...
    // Save map to CSV file: categoryLabel,totalWins
    public static void savePopularityMap(String filename, HashMap<String, Integer> map) {
        try {
            FileWriter writer = new FileWriter(filename);
            writer.write("categoryLabel,totalWins\n");
            for (String key : map.keySet()) {
                int val = map.get(key);
